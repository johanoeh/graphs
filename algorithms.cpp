...
bool all_visited(adjacency_matrix & m) {
...
const int generate_random_int(int lower, int upper) {
...
void genenerate_graph(adjacency_matrix & m, std::size_t start_n) {
    ...
    while (!q.empty()) {
        ...
        for (std::size_t i = 0; i < nodes.size(); i++) {
            ...
            if (!m.is_visited(nodes[i])) {

                q.push(nodes[i]);
                const int rand_int = generate_random_int(1, MAX_WEIGHT);
                m.add(nodes[i], curr_n, rand_int);
                m.set_visited(nodes[i], true);
                break;
